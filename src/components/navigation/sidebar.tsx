...
const SidebarContent: FunctionComponent<TRSidebar> = ({
  children,
  className,
  ...props
}) => {
  const userRole = useUserStore(({ user }) => user?.role);
  const sidebarClose = useGlobalStore(({ SidebarClose }) => SidebarClose);
  ...
  return (
    <PanelLayout
      className={clsx(
        "sticky top-0 px-3 py-4 flex-1 flex flex-col justify-between w-24 md:min-w-52",
        className
      )}
      withBorder
      {...props}
    >
      ...
      {userRole && (
        <div className='flex flex-col justify-center items-start gap-2'>
          <Link href='/panel' className='w-full' onClick={sidebarClose}>
            <Button
              className={clsx(
          ...
          {["ADMIN", "SUPERVISOR"].includes(userRole) && (
            <Link href='/panel/users' className='w-full' onClick={sidebarClose}>
              <Button
                className={clsx(
                  "w-full justify-start relative transition-all pis-7 text-foreground",
                  activePath === "users"
...
const Sidebar: FunctionComponent<TRSidebar> = ({
  ...
  const sidebarStatus = useGlobalStore(({ sidebarStatus, SidebarToggle }) => ({
    sidebarStatus,
    SidebarToggle,
  }));
  ...
  return (
    <>
      ...
      <Sheet
        open={sidebarStatus.sidebarStatus}
        onOpenChange={sidebarStatus.SidebarToggle}
      >
        <SheetContent
          className='h-full flex px-0 max-w-48 w-full bg-transparent border-0 shadow-none'
          side={"right"}
        >
          <SidebarContent className='rounded-es-none rounded-ss-none' />
        </SheetContent>
      </Sheet>
    </>
  );
